...
| SBB-14 | Security headers/CSP, rate limiter, upload limits, least-privilege DB role, non-root API image; **gating** AppSec scanning — SAST (Semgrep) · SCA/secrets/IaC (Trivy) in CI (triaged baseline) + on-demand DAST (OWASP ZAP) | ABB-09, ABB-12 | ADR-0008, ADR-0051, ADR-0053, security-hardening.md |
...
| SBB-17 | Docker + docker-compose (api=web + worker + db + nginx edge, one image/role) + nginx edge; GitHub Actions CI; **tag-triggered release pipeline publishing versioned api/web images to GHCR (Buildx + Trivy image scan)** | ABB-12 | ADR-0008, ADR-0048, ADR-0052 |
