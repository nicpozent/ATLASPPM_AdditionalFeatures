...
export const EVALUATION: Evaluation = {
  ...
  notes: [
    ...
    "Dark mode (ADR-0056): a per-profile light/dark preference, toggled from the top bar and persisted in localStorage keyed by the selected identity. Implemented with CSS variables — the color.* design tokens became var(--atlas-…) references so the whole app re-skins with no per-component change and no global stylesheet; the light palette (and its gated WCAG-AA contrast) is unchanged. Charts keep a literal palette (SVG presentation attributes don't resolve var()).",
    "Labour rates (ADR-0055): the internal-labour rate card is need-to-know — each discipline (Dev/Infra/Architect/PM/PO) is visible and editable only by its owning managers plus CTO/CIO; the API returns only the disciplines the caller's role may see, so no hidden rate leaves the server.",
...
export const USER_STORY_SECTIONS: UserStorySection[] = [
  { title: "1. Access & Identity", stories: [
    ...
    { id: "AUTH-6", role: "User", want: "a dark mode I can toggle per profile", benefit: "each persona I use keeps my preferred appearance", acceptance: "a sun/moon toggle in the top bar flips light/dark app-wide via CSS variables; the choice persists in localStorage keyed by the selected identity (atlas.theme.<role>); the light palette and its gated WCAG-AA contrast are unchanged (ADR-0056)." },
  ]},
