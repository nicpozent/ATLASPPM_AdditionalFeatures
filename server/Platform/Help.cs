...
public static class Help
{
    ...
    static readonly (string Audience, string Title, string Summary, string Body)[] Guides =
    {
        ...
        ("all", "Switch between light and dark mode",
            "A per-profile appearance toggle in the top bar.",
            "Use the sun/moon button in the top bar (next to the role switcher) to switch the whole app between light and dark. The choice is remembered per profile: each identity you switch to keeps its own light/dark preference, so you can, for example, run one persona dark and another light. The setting is stored in your browser (no account change), and the light theme meets the same WCAG AA contrast the app is tested against."),
        ("all", "Who can see internal-labour rates",
            "Rates are region-scoped and need-to-know.",
            "The internal-labour rate card (My Team) is deliberately restricted. Each rate line is a discipline × region — e.g. Infra · Sweden, Dev · APAC, PM · CH — and each line is visible AND editable only by the managers who own that region, plus the CTO and CIO who see every line. A regional manager sees only their own region; a role that owns no line sees a 'rates are restricted' message. The server only sends the lines your identity may see, so nothing hidden reaches the browser. To manage a region you don’t currently see, switch to (or be assigned) the owning role."),
        ...
        ("admin", "Set the idle-logout timeout",
            "Sign out unattended sessions after inactivity.",
            "Atlas signs an authenticated session out after a period of no activity (mouse, keyboard, scroll or touch), so an unattended workstation can't be used. The default is 15 minutes.\n1. Set VITE_AUTH_IDLE_MINUTES for the front end (default 15; clamped to 1–480). A typo or 0 falls back to 15.\n2. The timer resets on any interaction and is shared across the user's open tabs — they log out together.\n3. This applies only when SSO is enabled; it complements (does not replace) the access token's own expiry, which the API always enforces. For a hard server-side limit, shorten the token lifetime in Entra."),
        ...
        ("admin", "Connect Jira & Azure DevOps",
            "Discovery, import and work-item sync.",
            "1. In Integrations, add the connector credentials (Jira: email + API token; Azure DevOps: organisation + PAT — see the Azure DevOps setup guide for the PAT scopes).\n2. Use ‘Test connection’ on each card to confirm access.\n3. Jira — on a project, set its Jira project key. A board id is optional: with a board you also get sprints; without one, issues import by project key alone (a Jira ‘space’ maps fine).\n4. Azure DevOps — use ‘Discover from Azure DevOps’ to import/map an ADO project to a new or existing Atlas project (optionally under a program).\n5. Sync: ‘Sync now’ on each card pulls all mapped projects and runs in the background, so a large portfolio can’t time out — a toast reports progress and the counts when it finishes. ADO pulls iterations→sprints and work items→epics/tasks (Epics become epics; everything else becomes tasks; no iteration ⇒ backlog). Both connectors are one-way and idempotent — re-syncing never duplicates or touches manual rows. Assignees not synced from Entra are flagged so you can onboard them."),
        ...
        ("admin", "Enable notification email",
            ...
            "Atlas sends notification email through Microsoft Graph with an app-only (application) Mail.Send permission — no user mailbox password, no SMTP.\n\n1. Grant + admin-consent the Graph Mail.Send application permission. In the portal: App registration → API permissions → Add → Microsoft Graph → Application permissions → Mail.Send → Add, then “Grant admin consent”. Or in PowerShell (Microsoft.Graph module):\n\n   Connect-MgGraph -Scopes \"Application.ReadWrite.All\",\"AppRoleAssignment.ReadWrite.All\"\n   $graph    = Get-MgServicePrincipal -Filter \"appId eq '00000003-0000-0000-c000-000000000000'\"\n   $mailSend = $graph.AppRoles | ? { $_.Value -eq 'Mail.Send' -and $_.AllowedMemberTypes -contains 'Application' }\n   $sp       = Get-MgServicePrincipal -Filter \"appId eq '<YOUR-APP-CLIENT-ID>'\"\n   New-MgServicePrincipalAppRoleAssignment -ServicePrincipalId $sp.Id -PrincipalId $sp.Id -ResourceId $graph.Id -AppRoleId $mailSend.Id\n\n2. (Recommended) Restrict which mailboxes the app may send as — otherwise app-only Mail.Send can send as ANY mailbox. In Exchange Online PowerShell:\n\n   New-ApplicationAccessPolicy -AppId <YOUR-APP-CLIENT-ID> -PolicyScopeGroupId atlas-senders@yourco.com -AccessRight RestrictAccess -Description \"Restrict Atlas to the sender mailbox\"\n\n3. Set Graph:TenantId / Graph:ClientId / Graph:ClientSecret and Notifications:SenderUpn (the mailbox mail is sent from; must be in the access-policy group above).\n4. Send a test notification and confirm delivery. Until this is configured, notifications still appear in-app; only email is skipped.\n\nFull step-by-step (portal + PowerShell, config keys, verification, troubleshooting): docs/email-graph-setup.md."),
        ...
        ("pm", "Use the Resource & Sprint Gantt",
            ...
            "1. Open Timeline/Gantt and choose the scope (Project or Program) and the view (Schedule, Resources, Sprints, category views).\n2. Sprints appear as a single duration bar you can expand to see their tasks — for every onboarded project (Jira, Azure DevOps or manual), past, current and future.\n3. Not seeing sprints under a project? Run its Jira/ADO sync (Integrations → Sync). For Jira you no longer need to enter a board id — Atlas discovers the project's scrum board by key and imports its sprints. You can also add a phase or sprint manually.\n4. Add milestones and see dependency arrows across the month grid; export when you need a static copy."),
        ...
        ("pm", "How task estimates count toward allocation",
            "Open task hours become bottom-up capacity load.",
            "Task estimate hours feed allocation, so utilisation reflects real committed work — not just planned %.\n1. On a project task, set an estimate (hours) and, ideally, a start/target date.\n2. Each open task's estimate is smoothed over its window (task dates, else the project window, else a rolling ~4 weeks) into a weekly % of the assignee's capacity (40 h/week = 100%).\n3. Per project, a person's load is the HIGHER of their planned team % and their task-derived % — the two never double-count. It's summed across projects into their Project % on Resources and the project's Team capacity.\n4. Done tasks stop consuming capacity. Someone with estimated tasks on a project shows up in its capacity panel even without a formal team assignment."),
        ("pmo", "Read capacity intelligence & staff by skill",
            "Over-allocation, headroom, demand-vs-capacity, and skills-based staffing.",
            "Resources → Capacity insight.\n• Top tiles: headcount, portfolio load %, over-allocated count, how many have ≥50% free, and unallocated people. The demand-vs-capacity bar compares total load to total capacity (headcount × 100%).\n• ‘Over-allocated’ lists everyone above 100% (ops + project + product); ‘Under-utilised’ lists people below 50%. ‘Capacity vs demand by department’ shows load per unit.\n• ‘Find someone to staff’: type a skill and a minimum free %, and Atlas returns people rated in that skill (from the My-Team matrix) who have spare capacity, ranked by free % then rating.\n• ‘My allocations’ shows your own current slices and free capacity (matched by your name).\nEverything is derived live from the same time-phased allocation as the Resources roster."),
        ...
        ("pm", "Track operational (run-the-business) work",
            ...
            "The Ops section is for run-the-business work that isn't project delivery (support, maintenance, monitoring, infrastructure).\n1. Create an Ops service, then add work items (type, priority, status, assignee, and an allocation %). Services can be edited, archived (hidden from the board without losing history — toggle ‘Show archived’ to see them, then Restore) or deleted.\n2. A work item's allocation rolls up into that person's Ops% on Resources — so BAU load counts against their capacity.\n3. Tag an item with an ‘impact project’ to show, on that project's Overview, the operational load pulling capacity off its delivery.\n4. ‘Import from Jira’ turns a Jira project (an ops/support space or board) into an Ops service and pulls its issues in as work items — re-importing the same key tops up new issues (idempotent); allocation stays 0 until you set it. Editing Ops needs the Operational-work capability."),
